...
fromNonempty nonempty =
    fromLists [] (Nonempty.head nonempty) (Nonempty.tail nonempty)


moveForward : SelectList a -> SelectList a
moveForward (SelectList before_ x after_) =
    case ( before_, x, after_ ) of
        ( [], current, [] ) ->
            SelectList [] current []

        ( b, current, first :: rest ) ->
            SelectList (b ++ [ current ]) first rest

        ( first :: rest, current, [] ) ->
            SelectList [] first (rest ++ [ current ])


moveBackward : SelectList a -> SelectList a
moveBackward (SelectList before_ x after_) =
    case ( before_, x, after_ ) of
        ( [], current, [] ) ->
            SelectList [] current []

        ( first, current, rest ) ->
            case List.unconsLast first of
                Just ( newCurrent, before__ ) ->
                    SelectList before__ newCurrent (current :: rest)

                Nothing ->
                    case List.unconsLast rest of
                        Just ( newCurrent, before__ ) ->
                            SelectList (current :: before__) newCurrent []
